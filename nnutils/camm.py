...
from nnutils.geom_utils import K2mat, gauss_mlp_skinning, get_interpolated_skinning_weights_torch, get_refined_bones_transforms_torch_batch, lbs, load_skeleton, mat2K, Kmatinv, K2inv, raycast, sample_xy,\
...
from nnutils.rendering import render_rays, render_rays_skel
from nnutils.loss_utils import eikonal_loss, rtk_loss, \
                            ...
                            compute_root_sm_2nd_loss, shape_init_loss, bone_init_loss, \
                            sdf_bone_loss
...
# extra_tag
# flags.DEFINE_string('extra_tag', 'default', 'extra tag')
...
flags.DEFINE_string('skeleton_file', '', 'path to skeleton pickle file')
...
flags.DEFINE_bool('use_bone_offset', False, 'whether to apply offset to bone initialization')
flags.DEFINE_bool('use_center_skeleton', False, 'whether to use center skeleton')
...
flags.DEFINE_bool('disable_delta', False,   'whether to disable delta rts in expmlp')

# optimization: hyperparams
flags.DEFINE_integer('num_epochs', 120, 'Number of epochs to train')
...
flags.DEFINE_float('use_scale_bones', 1, 'exp() is used so neg is larger help scale (num_bones, 10), -1.5 gave decent results')
flags.DEFINE_float('use_shape_factor', 0.1, 'used in _init_losses losses')
flags.DEFINE_bool('joints_only', False, 'whether to use bone centers only for skinning weights')
...
# DINO ViT features (turned off by default)
flags.DEFINE_bool('use_dino', False, 'whether to use DINO ViT features')
flags.DEFINE_integer('dino_embed_dim', 16, 'canonical feature embedding dimension')
...
flags.DEFINE_bool('use_hs', False, 'whether to use hierarchical_softmax (only training)')
flags.DEFINE_float('sdf_bone_reg', 0, 'wheter to use sdf bone loss')
flags.DEFINE_float('use_bone_init_loss', 0, 'wheter to use bone_init_loss')
flags.DEFINE_bool('use_embed_bones_all', False, 'if to embed only the centers or whole default - centers')
flags.DEFINE_float('skeleton_based_transform_loss', 0, 'whether to use a loss to enforce nerf_body_rts outputs same transform as refined transforms from skeleton')
flags.DEFINE_float('skeleton_bone_residual', 0, 'the maximum change of length for each skeleton bone')
...
class camm(nn.Module):
    def __init__(self, opts, data_info):
        ...
        if opts.flowbw:
            ...
            bones= generate_bones(self.num_bones, self.num_bones, 0, self.device, use_bone_offset=opts.use_bone_offset)
            ...
            self.use_hs = opts.use_hs

            if opts.skeleton_file != '':
                if opts.skeleton_bone_residual > 0:
                    self.skeleton = load_skeleton(opts.skeleton_file, self.device, residual_update=True)
                    # self.skel_bone_residuals = torch.randn(self.skeleton.joint_centers.shape[0]-1,)
                    self.skel_bone_residuals = torch.zeros((self.skeleton.joint_centers.shape[0]-1,))
                    self.skel_bone_residuals = nn.Parameter(self.skel_bone_residuals, requires_grad=True)
                    # self.nerf_models['skel_bone_residuals'] = self.skel_bone_residuals
                else:
                    self.skeleton = load_skeleton(opts.skeleton_file, self.device, residual_update=False)
                self.nerf_models['skeleton'] = self.skeleton
...
#                                    D=5,W=128,
        ...
        if opts.root_opt:
            ...
            if self.root_basis == 'cnn':
                ...
                self.nerf_root_rts = RTExpMLP(self.num_fr, 
                                  ...
                                  delta=self.use_cam, disable_delta_rts=opts.disable_delta)
        ...
        if opts.use_embed:
            if opts.use_dino:
                self.num_feat = opts.dino_embed_dim
    ...
    def forward_default(self, batch):
        ...
        if opts.skeleton_file != '':
            ...
            if opts.skeleton_bone_residual > 0:
                clipped_residual = torch.tanh(self.skel_bone_residuals) * opts.skeleton_bone_residual
                self.skeleton.update_skeleton_with_residuals(clipped_residual)
            bone_to_skeleton_pairs = get_interpolated_skinning_weights_torch(self.skeleton, bones_rst)
            rendered, rand_inds = self.nerf_render_skel(rtk, kaug, embedid, bone_to_skeleton_pairs, self.bones,
                    nsample=opts.nsample, ndepth=opts.ndepth)
        else:
            rendered, rand_inds = self.nerf_render(rtk, kaug, embedid, 
                    nsample=opts.nsample, ndepth=opts.ndepth)

        if 'bones_dfm' in rendered.keys():
            aux_out['bones_dfm'] = rendered['bones_dfm'].view(-1,opts.num_bones,10)
        ...
        if opts.skeleton_based_transform_loss > 0:
            skeleton_based_transform_loss = rendered['regulate_rts_loss'] * opts.skeleton_based_transform_loss
            total_loss = total_loss + skeleton_based_transform_loss
            aux_out['regulate_rts_loss'] = skeleton_based_transform_loss
        
        if opts.use_proj and 'proj_err' in rendered.keys():
        ...
        if opts.lbs and opts.sdf_bone_reg> 0:
            bones_rst = self.bones
            bones_rst,_ = correct_bones(self, bones_rst)
            bone_loss = sdf_bone_loss(bones_rst * opts.use_scale_bones, self.embedding_xyz, self.nerf_vis, opts.use_embed_bones_all)
            # Also check for centre only embed in loss
            bone_loss = opts.sdf_bone_reg*bone_loss
            aux_out['sdf_bone_loss'] = bone_loss
            total_loss = total_loss + bone_loss
...
#            unc_accumulated = unc_rgb + unc_sil
    ...
    def forward_warmup_shape(self, batch):
        ...
        # loss
        shape_factor = opts.use_shape_factor
        aux_out={}
        ...
        if opts.use_bone_init_loss > 0:
            bones_rst = self.bones
            bones_rst,_ = correct_bones(self, bones_rst)

            bone_sdf_loss = bone_init_loss(bones_rst[:,:3]*shape_factor, \
                                self.nerf_coarse, self.embedding_xyz, 
                bound_factor=opts.bound_factor * 1.2, use_ellips=opts.init_ellips)
            aux_out['bone_sdf_loss'] = bone_sdf_loss
            total_loss = total_loss + bone_sdf_loss
    ...
    def nerf_render_skel(self, rtk, kaug, embedid, bone_to_skeleton_pairs, bones, nsample=256, ndepth=128):
        opts=self.opts
        # render rays
        if opts.debug:
            torch.cuda.synchronize()
        ...
        rand_inds, rays, frameid, errid = self.sample_pxs(bs, nsample, Rmat, Tmat, Kinv,
        self.dataid, self.frameid, self.frameid_sub, self.embedid,self.lineid,self.errid,
        self.imgs, self.masks, self.vis2d, self.flow, self.occ, self.dp_feats, bone_to_skeleton_pairs=bone_to_skeleton_pairs, bones=bones)
        # rand_inds, rays, frameid, errid = self.sample_pxs(bs, nsample, Rmat, Tmat, Kinv,
        # self.dataid, self.frameid, self.frameid_sub, self.embedid,self.lineid,self.errid,
        # self.imgs, self.masks, self.vis2d, self.flow, self.occ, self.dp_feats)
        self.frameid = frameid # only used in loss filter
        ...
        for i in range(0, bs_rays, opts.chunk):
            ...
            if self.progress > opts.fine_steps:
                ...
                self.use_fine = False
            rendered_chunks = render_rays_skel(self.nerf_models,
                        self.embeddings,
                        rays_chunk,
                        bone_to_skeleton_pairs,
                        N_samples = ndepth,
                        use_disp=False,
                        perturb=opts.perturb,
                        noise_std=opts.noise_std,
    ...
    def sample_pxs(self, bs, nsample, Rmat, Tmat, Kinv,
                   ...
                   imgs, masks, vis2d, flow, occ, dp_feats, bone_to_skeleton_pairs=None, bones=None):
        """
        ...
        if 'skeleton' in self.nerf_models.keys() and bone_to_skeleton_pairs is not None:
            self.update_rays_skel(rays, bs>1, dataid, frameid_sub, frameid, xys, Kinv, bone_to_skeleton_pairs, bones)
        else:
            self.update_rays(rays, bs>1, dataid, frameid_sub, frameid, xys, Kinv)
        if 'bones' in self.nerf_models.keys():
            if 'skeleton' in self.nerf_models.keys() and bone_to_skeleton_pairs is not None:
                bones_rst, _ = correct_bones(self, self.nerf_models['bones'])
        ...
        if opts.use_embed:
            if opts.use_dino:
                rays['feats_at_samp']=torch.gather(dp_feats[batch_map][...,0], 2, 
                    rand_inds[:,None].repeat(1,opts.dino_embed_dim,1))[:,None][...,0]            
            else:
                rays['feats_at_samp']=torch.gather(dp_feats[batch_map][...,0], 2, 
    ...
    def update_rays_skel(self, rays, is_pair, dataid, frameid_sub, embedid, xys, Kinv, bone_to_skeleton_pairs, bones):
        """
        """
        opts = self.opts
        # append target frame rtk
        embedid = embedid.long().to(self.device)
        # print('embedid',embedid.shape)
        if is_pair:
            rtk_vec = rays['rtk_vec'] # bs, N, 21
            rtk_vec_target = rtk_vec.view(2,-1).flip(0)
            rays['rtk_vec_target'] = rtk_vec_target.reshape(rays['rtk_vec'].shape)
            ...
            embedid_target = embedid.view(2,-1).flip(0).reshape(-1)
            # print('embedid target',embedid_target.shape)
            if opts.flowbw:
                time_embedded_target = self.pose_code(embedid_target)[:,None]
                rays['time_embedded_target'] = time_embedded_target.repeat(1,
                                                            rays['nsample'],1)
                ...
                # get canonical bones
                bones_rst, bone_rts_rst = correct_bones(self, bones)
                self.nerf_models['bones_rst']=bones_rst

                if opts.nerf_skin:
                    nerf_skin = self.nerf_skin
                else:
                    nerf_skin = None
                ...
                skin_forward = gauss_mlp_skinning(self.skeleton.joint_centers[:,None], self.embedding_xyz, bones_rst, 
                                    rest_pose_code, nerf_skin, use_hs=opts.use_hs, skin_aux=self.skin_aux, joints_only=opts.joints_only)

                unique_ids = torch.unique(embedid)

                transforms_dict = {}
                num_pts = 1
                num_joints = self.skeleton.joint_centers.shape[0]
                unique_bone_rts_fw = self.nerf_body_rts(unique_ids)
                unique_bone_rts_fw = correct_rest_pose(opts, unique_bone_rts_fw, bone_rts_rst)
                # print('unique bone rts',unique_bone_rts_fw.shape)

                unique_bone_rts_fw_skel = torch.repeat_interleave(unique_bone_rts_fw, num_joints, dim=0)
                skin_forward_skel = skin_forward.repeat(unique_ids.shape[0],1,1)
                skeleton_repeated = (self.skeleton.joint_centers[:,None]).repeat(unique_ids.shape[0],1,1)
                pts_dfm_all, _ = lbs(bones_rst, unique_bone_rts_fw_skel,
                                    skin_forward_skel, skeleton_repeated, backward=False)
                # print('overall',pts_dfm_all.shape)
                regulated_locations_all = self.skeleton.get_proper_transforms_batch(pts_dfm_all.squeeze(1),num_pts)
                # print('regulated locs all',regulated_locations_all.shape)
                refined_transform_all = get_refined_bones_transforms_torch_batch(bones_rst, regulated_locations_all, bone_to_skeleton_pairs, num_pts, self.device)
                # print('refined transform all',refined_transform_all.shape)
                # start_time1 = time.time()
                for i in range(unique_ids.shape[0]):
                    embedid_id = unique_ids[i].item()
                    # bone_rts_fw = unique_bone_rts_fw[i:i+1].repeat(num_joints,1,1)
                    # print('shapes',bone_rts_fw.shape, bones_rst.shape, skin_forward.shape, self.skeleton.joint_centers[:,None].shape)
                    # pts_dfm, _ = lbs(bones_rst, bone_rts_fw,
                    #                 skin_forward, self.skeleton.joint_centers[:,None], backward=False)
                    # # print('pts_dfm shape',pts_dfm.shape)
                    # print('same deformed locs',torch.allclose(pts_dfm_all[i*num_joints:(i+1)*num_joints],pts_dfm,atol=1e-8))
                    # # print('individual transform',time.time()-start_time)
                    # # pts_dfm = pts_dfm_all[i*num_joints:(i+1)*num_joints]
                    # _, regulated_locations = self.skeleton.get_proper_transforms(pts_dfm.squeeze(1),num_pts)
                    # print('same regulated locs',torch.allclose(regulated_locations_all[i],regulated_locations,atol=1e-8))
                    # # print('get proper transforms',time.time()-start_time)
                    # # regulated_locations = regulated_locations_all[i]
                    # refined_transform = get_refined_bones_transforms_torch(bones_rst, regulated_locations, bone_to_skeleton_pairs, num_pts, self.device)
                    # # print('gt refined transform',refined_transform.shape)
                    # # refined_transform = get_refined_bones_transforms_torch(bones_rst, regulated_locations, bone_to_skeleton_pairs, num_pts, self.device)
                    # print('same refined transform',torch.allclose(refined_transform_all[i].view(1,1,-1),refined_transform,atol=1e-5))
                    # print(refined_transform_all[i].view(1,1,-1),refined_transform)
                    transforms_dict[embedid_id] = refined_transform_all[i].view(1,1,-1)

                # print(transforms_dict)
                # print('refine transform',time.time()-start_time1)
                # bone_rts_target_raw = self.nerf_body_rts(embedid_target)
                # bone_rts_target_raw_skel = correct_rest_pose(opts, bone_rts_target_raw, bone_rts_rst)
                # bone_rts_target_raw_skel = torch.repeat_interleave(bone_rts_target_raw, self.skeleton.joint_centers.shape[0], dim=0)

                # bone_rts_raw = self.nerf_body_rts(embedid)
                # bone_rts_raw_skel = correct_rest_pose(opts, bone_rts_raw, bone_rts_rst)
                # bone_rts_raw_skel = torch.repeat_interleave(bone_rts_raw, self.skeleton.joint_centers.shape[0], dim=0)
                # start_time2 = time.time()
                bone_rts_target = torch.zeros(embedid_target.shape[0],1,bones.shape[0]*12).to(self.device)
                bone_rts = torch.zeros(embedid.shape[0],1,bones.shape[0]*12).to(self.device)
                for i in range(embedid_target.shape[0]):
                    # idx = i*num_joints
                    # pts_dfm, _ = lbs(bones_rst, bone_rts_target_raw_skel[idx:idx+num_joints,:,:],
                    #                 skin_forward, self.skeleton.joint_centers[:,None], backward=False)
                    # _, regulated_locations = self.skeleton.get_proper_transforms(pts_dfm.squeeze(1),num_pts)
                    # refined_transform = get_refined_bones_transforms_torch(bones_rst, regulated_locations, bone_to_skeleton_pairs, num_pts, self.device)
                    # bone_rts_target[i:i+1,:,:] = refined_transform
                    bone_rts_target[i:i+1,:,:] = transforms_dict[embedid_target[i].item()]

                    # pts_dfm, _ = lbs(bones_rst, bone_rts_raw_skel[idx:idx+num_joints,:,:],
                    #                 skin_forward, self.skeleton.joint_centers[:,None], backward=False)
                    # _, regulated_locations = self.skeleton.get_proper_transforms(pts_dfm.squeeze(1),num_pts)
                    # refined_transform = get_refined_bones_transforms_torch(bones_rst, regulated_locations, bone_to_skeleton_pairs, num_pts, self.device)
                    # bone_rts[i:i+1,:,:] = refined_transform
                    bone_rts[i:i+1,:,:] = transforms_dict[embedid[i].item()]
                # print('assignment',time.time()-start_time2)
                rays['bone_rts_target'] = bone_rts_target.repeat(1,rays['nsample'],1)
                rays['bone_rts'] = bone_rts.repeat(1,rays['nsample'],1)

                # also compute unregulated transforms if skeleton based transforms loss is enabled
                if self.opts.skeleton_based_transform_loss > 0:
                    unregulated_bone_rts = self.nerf_body_rts(embedid)
                    unregulated_bone_rts = unregulated_bone_rts.repeat(1,rays['nsample'],1)

                    unregulated_bone_rts_target = self.nerf_body_rts(embedid_target)
                    unregulated_bone_rts_target = unregulated_bone_rts_target.repeat(1,rays['nsample'],1)

                    rays['unregulated_bone_rts'] = correct_rest_pose(opts, unregulated_bone_rts, bone_rts_rst)
                    rays['unregulated_bone_rts_target'] = correct_rest_pose(opts, unregulated_bone_rts_target, bone_rts_rst)
                    # print('checking shapes1',rays['bone_rts_target'].shape,rays['bone_rts'].shape,rays['unregulated_bone_rts'].shape,rays['unregulated_bone_rts_target'].shape)
    ...
    def convert_line_input(self, batch):
        ...
        if opts.use_dino:
            self.dp_feats     = batch['dino_feat_rsmp'].view(bs,2,opts.dino_embed_dim,-1).permute(1,0,2,3).reshape(bs*2,opts.dino_embed_dim,-1,1).to(device)
