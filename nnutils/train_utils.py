...
from nnutils.geom_utils import bone_transform, get_interpolated_skinning_weights_torch, lbs, reinit_bones, warp_bw, warp_fw, vec_to_sim3,\
                               ...
                               correct_bones, correct_rest_pose, get_bones_2d, get_interpolated_skinning_weights, \
                               warp_fw_skel
from nnutils.nerf import grab_xyz_weights
from ext_utils.flowlib import flow_to_image
from utils.io import mkdir_p
from nnutils.vis_utils import get_skeleton, image_grid
...
class v2s_trainer():
    ...
    def define_model(self, data_info):
        ...
        self.model = camm.camm(opts, data_info)
    ...
    def init_dataset(self):
        ...
        opts_dict['use_dino'] = opts.use_dino
    ...
    def init_training(self):
        ...
        for name,p in self.model.named_parameters():
            ...
            elif 'skel_bone_residuals' in name:
                params_skel_residuals.append(p)
            else: continue
            # if opts.local_rank==0:
        ...
        lr_nerf_root_rts*opts.learning_rate, # params_root_code
                      ...
                      10*opts.learning_rate, # params_ks
                         ...
                         opts.learning_rate, # params_skel_residuals
            ],
    ...
    def eval(self, idx_render=None, dynamic_mesh=False): 
        ...
        with torch.no_grad():
            ...
            for j in range(0, len(idx_render), chunk):
                ...
                if opts.use_proj and 'proj_err' in rendered_seq.keys():
            ...
            if opts.lbs:
                ...
                if mesh_rest.vertices.shape[0]>100:
                    ...
                    skins = gauss_mlp_skinning(rest_verts[None], 
                            ...
                            nerf_skin, use_hs=self.opts.use_hs, skin_aux=self.model.skin_aux, joints_only=opts.joints_only)[0]
            ...
            if 'bones_dfm' in rendered_seq.keys():
                del rendered_seq['bones_dfm']
            # save images
    ...
    def eval_skel(self, idx_render=None, dynamic_mesh=False, skeleton=None, bone_to_skeleton_pairs=None): 
        """
        idx_render: list of frame index to render
        dynamic_mesh: whether to extract canonical shape, or dynamic shape
        """
        ...
        with torch.no_grad():
            self.model.eval()
            # run marching cubes on canonical shape
            mesh_dict_rest = self.extract_mesh_skel(self.model, opts.chunk, \
                                         opts.sample_grid3d, opts.mc_threshold, skeleton=skeleton)
            ...
            for j in range(0, len(idx_render), chunk):
                ...
                for idx in range(len(idx_chunk)):
                    ...
                    if dynamic_mesh:
                        if not opts.queryfw:
                           mesh_dict_rest=None 
                        mesh_dict = self.extract_mesh_skel(self.model,opts.chunk,
                                            opts.sample_grid3d, opts.mc_threshold,
                                        embedid=embedid, mesh_dict_in=mesh_dict_rest, skeleton=skeleton, bone_to_skeleton_pairs=bone_to_skeleton_pairs)
                        mesh=mesh_dict['mesh']
                        # if mesh_dict_rest is not None and opts.ce_color:
                        #     mesh.visual.vertex_colors = mesh_dict_rest['mesh'].\
                        #            visual.vertex_colors # assign rest surface color
                        ...
                        # save skeleton
                        if 'skeleton' in mesh_dict.keys():
                            skel = mesh_dict['skeleton'].cpu().numpy()
                            aux_seq['skeleton'].append(skel)
                        # save bones
                        if 'bones' in mesh_dict.keys():
                            bone = mesh_dict['bones'][0].cpu().numpy()
                            aux_seq['bone'].append(bone)
            ...
            if opts.local_rank==0:
                ...
                assert skeleton is not None, 'skeleton should not be None in evaluation'
                canonical_skel = get_skeleton(skeleton.joint_centers, skeleton.joint_connections)
                canonical_skel_path = '%s/skel_rest-%02d.obj'%(self.save_dir,suffix_id)
                canonical_skel.export(canonical_skel_path)
        ...
        for epoch in range(0, self.num_epochs):
            ...
            if opts.skeleton_file != '':
                if opts.skeleton_bone_residual > 0:
                    skel = get_skeleton(self.model.skeleton.joint_centers, self.model.skeleton.joint_connections)
                    # skel.export('saved_canonical_skel.obj')
                    clipped_residual = torch.tanh(self.model.skel_bone_residuals) * opts.skeleton_bone_residual
                    # clipped_residual = torch.ones_like(clipped_residual) * 0.01
                    # print('clipped residual in train()',clipped_residual) 
                    self.model.skeleton.update_skeleton_with_residuals(clipped_residual)
                    # skel = get_skeleton(self.model.skeleton.joint_centers, self.model.skeleton.joint_connections)
                    # skel.export('saved_canonical_skel_residuals.obj')
                    # print('saved both skeleton!!!!!!!!!!!!!!')
                    rendered_seq, aux_seq = self.eval_skel(skeleton=self.model.skeleton)
                else:
                    rendered_seq, aux_seq = self.eval_skel(skeleton=self.model.skeleton)                    
            else:
            ...
            if opts.skeleton_bone_residual > 0:
                self.model.skeleton.reset_skeleton()
    ...
    def train_one_epoch(self, epoch, log, warmup=False):
        ...
        for i, batch in enumerate(dataloader):
            ...
            if 'bones_dfm' in aux_out.keys():
                del aux_out['bones_dfm']
            ...
            if not warmup and opts.skeleton_bone_residual > 0:
                self.model.skeleton.reset_skeleton()
    ...
    def reset_hparams(self, epoch):
        ...
        if opts.lbs and opts.model_path=='' and \
            ...
            reinit_bones(self.model.module, mesh_rest, opts.num_bones, opts.use_center_skeleton)
    ...
    def broadcast(self):
        ...
        if self.opts.skeleton_bone_residual > 0:
            dist.broadcast(self.model.module.skel_bone_residuals,0)
    ...
    def clip_grad(self, aux_out):
        ...
        if self.opts.freeze_coarse:
            ...
            # freeze skinning 
            self.zero_grad_list(grad_bones) # freeze bones ? # # TODO 
            self.zero_grad_list(grad_skin_aux)
        ...
        aux_out['skel_residuals_g']= clip_grad_norm_(grad_skel_residuals,.1*clip_scale)
    ...
    def render_vid(model, batch):
        ...
        if opts.skeleton_file != '':
            ...
            bones_rst, _ = correct_bones(model, bones_rst)
            bone_to_skeleton_pairs = get_interpolated_skinning_weights_torch(model.skeleton, bones_rst)
            rendered, _ = model.nerf_render_skel(rtk, kaug, embedid, bone_to_skeleton_pairs, model.bones, ndepth=opts.ndepth)        
        else:
    ...
    @staticmethod
    def extract_mesh_skel(model,chunk,grid_size,
                      #threshold = -0.005,
                      threshold = -0.002,
                      #threshold = 0.,
                      embedid=None,
        ...
        if embedid is not None and opts.queryfw:
            mesh = mesh_dict_in['mesh'].copy()
            vertices = mesh.vertices
            vertices, mesh_dict = warp_fw_skel(opts, model, mesh_dict, 
                                           vertices, embedid, skeleton, bone_to_skeleton_pairs)
            mesh.vertices = vertices
