...
def render_rays_skel(models,
    ...
    if use_fine: # sample points for fine model
        ...
        #  not loss:   'depth_rnd', 'pts_pred', 'pts_exp'
        with torch.no_grad():
            _, weights_coarse = inference_deform_skel(xyz_sampled, rays, models, 
                              chunk, N_samples,
                              N_rays, embedding_xyz, rays_d, noise_std,
                              obj_bound, dir_embedded, z_vals,
                              img_size, progress,opts,bone_to_skeleton_pairs,fine_iter=False)
    ...
    result, _ = inference_deform_skel(xyz_sampled, rays, models, 
                          chunk, N_samples,
                          N_rays, embedding_xyz, rays_d, noise_std,
                          obj_bound, dir_embedded, z_vals,
                          img_size, progress,opts,bone_to_skeleton_pairs,render_vis=render_vis)
...
def inference_deform(xyz_coarse_sampled, rays, models, chunk, N_samples,
    ...
    if 'flowbw' in models.keys():
        ...
        skin_backward = gauss_mlp_skinning(xyz_coarse_sampled, embedding_xyz, 
                    bones_dfm, time_embedded, nerf_skin, use_hs=opts.use_hs, skin_aux=skin_aux, joints_only=opts.joints_only)
        ...
        if fine_iter:
            ...
            skin_forward = gauss_mlp_skinning(xyz_coarse_sampled, embedding_xyz, 
                        bones_rst,rest_pose_code,  nerf_skin, use_hs=opts.use_hs, skin_aux=skin_aux, joints_only=opts.joints_only)
    ...
    if fine_iter:
        if opts.use_corresp:
            ...
            pts_target = kp_reproj(pts_exp, models, embedding_xyz, rays, use_hs=opts.use_hs, 
                                to_target=True, joints_only=opts.joints_only) # N,1,2
        ...
        if 'feats_at_samp' in rays.keys():
            ...
            proj_err = kp_reproj_loss(pts_pred, xys, models, 
                    embedding_xyz, rays, use_hs=opts.use_hs, joints_only=opts.joints_only)
...
def inference_deform_skel(xyz_coarse_sampled, rays, models, chunk, N_samples,
                         N_rays, embedding_xyz, rays_d, noise_std,
                         obj_bound, dir_embedded, z_vals,
                         img_size, progress,opts,bone_to_skeleton_pairs, fine_iter=True, 
                         render_vis=False):
    """
    fine_iter: whether to render loss-related terms
    render_vis: used for novel view synthesis
    ...
    if 'flowbw' in models.keys():
        ...
        # direct supervision from refined transforms
        if 'unregulated_bone_rts' in rays.keys():
            refinement_loss = torch.nn.functional.mse_loss(rays['unregulated_bone_rts'], rays['bone_rts'])
        ...
        if fine_iter:
            ...
            if opts.dist_corresp and 'bone_rts_target' in rays.keys():
                ...
                if 'unregulated_bone_rts_target' in rays.keys():
                    refinement_loss += torch.nn.functional.mse_loss(rays['unregulated_bone_rts_target'], rays['bone_rts_target'])
    ...
    if 'bones' in models.keys():
        result['bones_dfm'] = bones_dfm
        if 'unregulated_bone_rts' in rays.keys():
            result['regulate_rts_loss'] = refinement_loss
