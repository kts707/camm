...
dino_base_path = 'database/DAVIS/DINO_Pixels/Full-Resolution/'
...
def main(_):
    ...
    for i, batch in enumerate(dataset):
        ...
        print(save_dir_t)
        dino_save_dir = '%s/%s'%(dino_base_path, seqname_sub)
        dino_save_dir_t = '%s/%d_%s'%(dino_save_dir, dt, frameid_sub)
        ...
        for idy in range(opts.img_size):
            ...
            dino_feat = np.load('%s/%04d.npy'%(dino_save_dir_t, idy))
            dict_px['dino_feat_rsmp'] = dino_feat
