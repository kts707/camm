...
def get_bones_mesh(bones, len_max):
    B = len(bones)
    elips_list = []
    elips = trimesh.creation.uv_sphere(radius=len_max/20,count=[16, 16])
    # remove identical vertices
...
class LineDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.use_dino:
            del elem['dp_feat_rsmp']
            del elem['dp']
            if 'dino_feat_rsmp' not in elem.keys():
                dino_save_dir = self.imglist[0].replace('JPEGImages', 'DINO_Pixels').rsplit('/',1)[0]
                dino_data_path = '%s/%d_%05d/%04d.npy'%(dino_save_dir, dframe, idt, idy)
                dino_feat = np.load(dino_data_path)
                elem['dino_feat_rsmp'] = dino_feat
        
        if not self.use_dino and 'dino_feat_rsmp' in elem.keys():
            del elem['dino_feat_rsmp']
...
class VidDataset(base_data.BaseDataset):
    ...
    def __init__(self, opts, filter_key=None, imglist=None, can_frame=0,
        ...
        # if 'mask_only' in opts.keys():
        #     self.mask_only = opts['mask_only']
        # else:
